...
public class ResearcherAPI extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String result = researcherObj.insertResearcher(request.getParameter("researcherCode"), 
				request.getParameter("researcherName"), 
				request.getParameter("Email"), 
				Integer.parseInt(request.getParameter("contactNo")), 
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String result = researcherObj.updateResearcher(param.get("hidresearcherIDSave").toString(),
				param.get("researcherCode").toString().toString().replace("+", " "),     
		 		param.get("researcherName").toString().toString().replace("+", " "),        
		 		param.get("Email").toString().toString().replace("%40", "@"),        
		 		Integer.parseInt(param.get("contactNo").toString()),
		 		param.get("projectCategory").toString().toString().replace("+", " ") );
